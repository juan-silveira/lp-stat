...
async function main() {  
   ...
   const BOBA_CHEF_ADDR = "0x61EF3199B26103D96CCFcF04073e0375B4C842eC";
   const rewardTokenTicker = "BOBA";
   const BOBA_CHEF = new ethers.Contract(BOBA_CHEF_ADDR, BOBA_CHEF_ABI, App.provider);

   const rewardsPerWeek = await BOBA_CHEF.bobaPerBlock() /1e18
        * 604800 / 3;

    const tokens = {};
    const prices = await getBscPrices();

    await loadBscChefContract(App, tokens, prices, BOBA_CHEF, BOBA_CHEF_ADDR, BOBA_CHEF_ABI, rewardTokenTicker,
        "boba", null, rewardsPerWeek, "pendingBoba");
